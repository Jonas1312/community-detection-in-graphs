...
for i in x:  # dense graph
    ...
    probability_matrix = np.full((n_communities, n_communities), cout) + np.diag(
    ...
    spectral_labels, eigvals, eigvects, W = SpectralClustering(n_communities, ModularityMatrix(sbm.adjacency_matrix),
                                                               "ModularityMatrix")  # spectral clustering
    nmi = normalized_mutual_info_score(sbm.community_labels, spectral_labels)
    y_modularity.append(nmi)
    spectral_labels, eigvals, eigvects, W = SpectralClustering(n_communities, LaplacianMatrix(sbm.adjacency_matrix),
                                                               "LaplacianMatrix")  # spectral clustering
    nmi = normalized_mutual_info_score(sbm.community_labels, spectral_labels)
